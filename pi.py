    ...
    while True:
        ...
        if True==SendButton:
            ...
            if str(Data)=='80':
                ...
                FileRead = open("DataBase\chat.txt","r")
                Data = FileRead.read()
                FileRead.close()
                FileHistory = open("DataBase\chat.txt","w")
                NewData = int(Data) + 1
                NewData = str(NewData)
...
try:
    with open("facebook.txt", "r") as FileDetailsFacebook:
        IdPassDetail = FileDetailsFacebook.read()
    
    IdPassDetail = str(IdPassDetail).strip()  # Remove leading/trailing whitespace

    if "," in IdPassDetail:
        Id, Passcode = IdPassDetail.split(",")
        Login(Id=Id, Passcode=Passcode)
        PopUpRemover()
        Introduction()
    else:
        print("Invalid IdPassDetail format. Expected format: 'Id,Passcode'")
